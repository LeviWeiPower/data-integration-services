...
var common = {
     ...
     * data 要填充的数据
     * ignoreIds 要忽略的字段，这个忽略的字段则不会填充数据
     */
    paddingForm: function (formId, labelName, data,ignoreIds) {
        $("#" + formId + " " + labelName).each(function () {
        	if(undefined == ignoreIds || null != ignoreIds || ignoreIds.indexOf($(this).attr("id")) == -1){
     ...
     * sortWay 默认是ASC（从小到大）
     */
    arrSortOfObjArr : function (arr,objFeild,sortWay){
    	
    	//从大到小
    	var compareDesc = function (obj1, obj2) {
		    var val1 = obj1[""+objFeild+""];
		    var val2 = obj2[""+objFeild+""];
		    if (val1 > val2) {
		        return -1;
		    } else if (val1 < val2) {
		        return 1;
    ...
    /**
     * 页面初始化权限，如果没有这个权限那么则进行进行隐藏
     */
    pageInitAuthoritry : function (){
    	//权限
    	var result = tool.ajax("/main/getCurLoginUserAuthorities","post");
    	//处理成功则刷新列表
    	if(CONSTANTS.HRS_OK == result.status ){
    		console.log("------------");
    		var resultLength = null == result.result || undefined == result.result ? 0 : result.result.length;
    		...
    		for (var i = 0; i < authorities.length; i++) {
    			//获取到页面权限标识数据
    			var authoritry = authorities[i];
    			var authoritryTagVal = $(authoritry).attr("authority");
    			
    			//标识，false（没有该权限），true（有该权限）
    			var flag = false;
    			for (var j = 0; j < resultLength; j++) {
    				//获取到结果对象
    				var resultObj = result.result[j];
    				var pageId = resultObj.pageId;
    				//判断该用户是否有这个权限
    				if(pageId == authoritryTagVal){
    					flag = true;
    					break;
    				}
    			}
